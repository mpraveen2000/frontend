...
import { CREATE_EMPLOYEE, GET_ALL_EMPLOYEE, UPDATE_EMPLOYEE } from "../query";
import { GET_ALL_SKILLS } from "../../skill/query";
import { DateTime } from "luxon";

interface PropsType {
  toggleDrawerVisible: () => void;
  editData: EmployeeTest | null | undefined;
}
...
export const CreateEmployee: React.FC<PropsType> = ({
  ...
  const skillsInfo = data?.getAllSkillTest;
  useEffect(() => {
    if (!editData) return;
    ...
    form.setFieldsValue({
      ...
      skillsId: editData?.skills?.map((skill: any) => skill?.id),
    });
  }, [form, editData]);
  const onFinish = async () => {
    ...
    try {
      ...
      const modifyEmployeeDto = {
        ...
        dob: values?.dob,
        doj: values?.doj,
        skillsTestId: values?.skillsId?.map((skill: any) => skill?.value),
      };

      if (editData?.id) {
        await updateEmployee({
          variables: {
            updateEmployeeTestId: editData?.id,
        ...
        await createEmployee({
          variables: {
            CreateEmployeeTest: modifyEmployeeDto,
          },
